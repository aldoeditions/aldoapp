    ...
    <nav className="flex flex-col gap-0.5">
      {LINKS.map(({ href, label, Icon }) => {
        ...
        return (
          ...
          >
            <Icon className={cn("shrink-0", active ? "text-white" : "text-white/60")} />
            {label}
          </Link>
        );
      })}
...
function Account({ name, avatarUrl }: { name: string; avatarUrl: string | null }) {
  return (
    <div className="flex items-center gap-2.5 border-t border-white/15 px-2 pt-4">
      <Avatar name={name} src={avatarUrl} size="sm" className="bg-white/20 text-white" />
...
export function PortalSidebar({
  ...
  return (
    <>
      {/* Desktop : sidebar bleue fixe */}
      <aside className="grain hidden w-64 shrink-0 flex-col bg-accent text-white md:flex">
        <div className="px-5 pb-6 pt-6">
          <Link href="/portail" className="flex items-center gap-2.5">
            <Logo className="h-6 text-white" />
            <span className="text-2xs font-semibold uppercase tracking-[0.16em] text-white/50">
              Espace artiste
            </span>
          </Link>
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="grain relative flex h-full w-72 max-w-[80%] flex-col bg-accent text-white shadow-float">
